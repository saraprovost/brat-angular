...
var VisualizerUI = function ($, window, undefined) {
  var VisualizerUI = function VisualizerUI(dispatcher, svg, showTooltip, overWriteModals) {
    var that = this;
    ...
    var hideNoDocMessage = function hideNoDocMessage() {
      clearTimeout(noSvgTimer);
      $('#no_svg_wrapper').hide(0);
      $('#source_files').show();
    };

    var showNoDocMessage = function showNoDocMessage() {
      clearTimeout(noSvgTimer);
    ...
    var docSortFunction = function docSortFunction(a, b) {
      // parent at the top
      if (a[2] === '..') return -1;
      if (b[2] === '..') return 1;
    ...
    var makeSortChangeFunction = function makeSortChangeFunction(sort, th, thNo) {
      $(th).click(function () {
        // TODO: avoid magic numbers in access to the selector
        // data (column 0 is type, 1 is args, rest is data)
        if (sort[0] === thNo + 1) sort[1] = -sort[1];else {
    ...
    var showPullupTrigger = function showPullupTrigger() {
    ...
    var displayMessages = function displayMessages(msgs) {
    ...
    var adjustToCursor = function adjustToCursor(evt, element, offset, top, right) {
    ...
    var displayComment = function displayComment(evt, target, comment, commentText, commentType, immediately) {
      var idtype;
      if (commentType) {
    ...
    var normInfoSortFunction = function normInfoSortFunction(a, b) {
      // images at the top
    ...
    var displaySpanComment = function displaySpanComment(evt, target, spanId, spanType, mods, spanText, commentText, commentType, normalizations) {
    ...
    var displayArcComment = function displayArcComment(evt, target, symmetric, arcId, originSpanId, originSpanType, role, targetSpanId, targetSpanType, commentText, commentType) {
    ...
    var displaySentComment = function displaySentComment(evt, target, commentText, commentType) {
      displayComment(evt, target, '', commentText, commentType);
    };

    var hideComment = function hideComment() {
      clearTimeout(displayCommentTimer);
      if (commentDisplayed) {
    ...
    initForm = function initForm(form, opts) {
      opts = opts || {};
    ...
    var selectElementInTable = function selectElementInTable(table, docname, mf) {
      table = $(table);
    ...
    var chooseDocument = function chooseDocument(evt) {
    ...
    var chooseDocumentAndSubmit = function chooseDocumentAndSubmit(evt) {
      chooseDocument(evt);
    ...
    */
    ...
    var docInputHandler = function docInputHandler(evt) {
      selectElementInTable('#document_select', $(this).val());
    };
    $('#document_input').keyup(docInputHandler);

    var fileBrowserSubmit = function fileBrowserSubmit(evt) {
      ...
      if (coll != _coll || doc != _doc || Util.paramArray(args.matchfocus) != matchFocus) {
        // something changed
    ...
    var showFileBrowser = function showFileBrowser() {
    ...
    var currentSelectorPosition = function currentSelectorPosition() {
      var pos;
    ...
    var addSpanTypesToSelect = function addSpanTypesToSelect($select, types, included) {
    ...
    var rememberNormDb = function rememberNormDb(response) {
    ...
    var setupSearchTypes = function setupSearchTypes(response) {
    ...
    var searchEventRoleChanged = function searchEventRoleChanged(evt) {
    ...
    var addEmptySearchEventRole = function addEmptySearchEventRole() {
    ...
    // deleting role rows
    var delSearchEventRole = function delSearchEventRole(evt) {
      $row = $(this).closest('tr');
      $row.remove();
    };
    ...
    var activeSearchTab = function activeSearchTab() {
      // activeTab: 0 = Text, 1 = Entity, 2 = Event, 3 = Relation, 4 = Notes, 5 = Load
      var activeTab = $('#search_tabs').tabs('option', 'selected');
      return ['searchText', 'searchEntity', 'searchEvent', 'searchRelation', 'searchNote', 'searchLoad'][activeTab];
    };

    var onSearchTabSelect = function onSearchTabSelect() {
    ...
    var applySearchResults = function applySearchResults(response) {
      if (!searchActive) {
    ...
    var searchFormSubmit = function searchFormSubmit(evt) {
    ...
    var updateSearchButtons = function updateSearchButtons() {
    ...
    var dataFormSubmit = function dataFormSubmit(evt) {
      dispatcher.post('hideForm');
      return false;
    };
    ...
    var optionsFormSubmit = function optionsFormSubmit(evt) {
      dispatcher.post('hideForm');
      return false;
    };
    ...
    var fillDisambiguatorOptions = function fillDisambiguatorOptions(disambiguators) {
    ...
    var moreInfoDialogSubmit = function moreInfoDialogSubmit(evt) {
      dispatcher.post('hideForm');
      return false;
    };
    ...
    var moveInFileBrowser = function moveInFileBrowser(dir) {
      ...
      if (newPos >= 0 && newPos < selectorData.items.length && selectorData.items[newPos][0] != "c") {
        ...
        dispatcher.post('allowReloadByURL');
        dispatcher.post('setDocument', [selectorData.items[newPos][2], selectorData.items[newPos][1]]);
      }
      return false;
    };
    ...
    var autoPaging = function autoPaging(on) {
    ...
    var resizeFunction = function resizeFunction(evt) {
      dispatcher.post('renderData');
    };

    var resizerTimeout = null;
    var onResize = function onResize(evt) {
    ...
    var collectionLoaded = function collectionLoaded(response) {
      if (response.exception) {
        if (response.exception == 'annotationCollectionNotFound' || response.exception == 'collectionNotAccessible') {
          // revert to last good
    ...
    var searchResultsReceived = function searchResultsReceived(response) {
      if (response.exception) {
    ...
    var clearSearch = function clearSearch(dontShowFileBrowser) {
      dispatcher.post('hideForm');
    ...
    var onDoneRendering = function onDoneRendering(coll, doc, args) {
      if (args && !args.edited) {
        var svgtop = $('svg').offset().top;
        var $inFocus = $('#svg animate[data-type="focus"]:first').parent();
        if ($inFocus.length) {
          $('html,body').animate({ scrollTop: $inFocus.offset().top - svgtop - window.innerHeight / 2 }, { duration: 'slow', easing: 'swing' });
        }
      }
      dispatcher.post('allowReloadByURL');
      if (!currentForm) {
    ...
    var onStartedRendering = function onStartedRendering() {
      hideForm();
      if (!currentForm) {
        $('#waiter').dialog('open');
      }
    ...
    var savedSVGreceived = function savedSVGreceived(response) {
      ...
      $.each(response.stored, function (storedNo, stored) {
        ...
        var $downloadLink = $('<a id="download_' + stored.name + '"' + ' target="' + stored.name + '"' + '>' + stored.name + '</a>');
    ...
    var invalidateSavedSVG = function invalidateSavedSVG() {
    ...
    var onNewSourceData = function onNewSourceData(sourceData) {
      ...
      $.each(sourceData.source_files, function (extNo, ext) {
        var $link = $('<a target="brat_search"/>').text(ext).attr('href', 'ajax.cgi?action=downloadFile&collection=' + encodeURIComponent(coll) + '&document=' + encodeURIComponent(doc) + '&extension=' + encodeURIComponent(ext) +
        // TODO: Extract the protocol version into somewhere global
        '&protocol=' + 1);
        $link.button();
    ...
    var gotCurrent = function gotCurrent(_coll, _doc, _args) {
      ...
      if (oldColl != coll) {
        ...
        var $collectionDownloadLink = $('<a target="brat_search"/>').text('Download tar.gz').attr('href', 'ajax.cgi?action=downloadCollection&collection=' + encodeURIComponent(coll)
        // TODO: Extract the protocol version into somewhere global
        + '&protocol=' + 1);
        ...
        var $cmpLink = $('<a target="_blank"/>').text('Comparison mode').attr('href', 'diff.xhtml?diff=' + encodeURIComponent(coll));
    ...
    var slideToggle = function slideToggle(el, show, autoHeight, bottom) {
      var el = $(el);
    ...
    var adjustFormToCursor = function adjustFormToCursor(evt, element) {
    ...
    var onSingleClick = function onSingleClick(evt) {
    ...
    var authFormSubmit = function authFormSubmit(evt) {
      dispatcher.post('hideForm');
    ...
    var noFileSpecified = function noFileSpecified() {
      // not (only) an error, so no messaging
      dispatcher.post('clearSVG');
      showFileBrowser();
    };

    var showUnableToReadTextFile = function showUnableToReadTextFile() {
      dispatcher.post('messages', [[['Unable to read the text file.', 'error']]]);
      dispatcher.post('clearSVG');
      showFileBrowser();
    };

    var showAnnotationFileNotFound = function showAnnotationFileNotFound() {
      dispatcher.post('messages', [[['Annotation file not found.', 'error']]]);
      dispatcher.post('clearSVG');
      showFileBrowser();
    };

    var showUnknownError = function showUnknownError(exception) {
      dispatcher.post('messages', [[['Unknown error: ' + exception, 'error']]]);
      dispatcher.post('clearSVG');
      showFileBrowser();
    };

    var reloadDirectoryWithSlash = function reloadDirectoryWithSlash(sourceData) {
    ...
    var annotationIsAvailable = function annotationIsAvailable() {
    ...
    var checkForDocumentChanges = function checkForDocumentChanges() {
        ...
        dispatcher.post('ajax', [opts, function (response) {
          if (data) {
            if (mtime != response.mtime) {
              ...
              if (documentChangesTimeout >= maxDocumentChangesTimeout) documentChangesTimeout = maxDocumentChangesTimeout;
            }
          }
        }]);
      } else {
    ...
    var configurationChanged = function configurationChanged() {
