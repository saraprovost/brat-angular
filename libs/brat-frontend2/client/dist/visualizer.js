...
var Visualizer = function ($, window, undefined) {
  ...
  var Visualizer = function Visualizer(dispatcher, svgId, webFontURLs) {
    ...
    var getTextAndSpanTextMeasurements = function getTextAndSpanTextMeasurements() {
      ...
      var textSizes = getTextMeasurements(chunkTexts, undefined, function (fragment, text) {
        if (fragment instanceof Fragment) {
          ...
          var endPos = lastChar < 0 ? startPos : text.getEndPositionOfChar(lastChar).x;
          fragment.curly = {
            from: startPos,
            to: endPos
          };
    ...
    var renderDataReal = function renderDataReal(sourceData) {
      ...
      $.each(data.chunks, function (chunkNo, chunk) {
        ...
        $.each(chunk.fragments, function (fragmentNo, fragment) {
          ...
          if (span.shadowClass && span.shadowClass.match('True_positive|False_positive|False_negative|AnnotationError|AnnotationWarning|AnnotatorNotes|Normalized|AnnotationIncomplete|AnnotationUnconfirmed|rectEditHighlight|EditHighlight_arc|MissingAnnotation|ChangedAnnotation ')) {
            shadowRect = svg.rect(fragment.group, bx - rectShadowSize, by - rectShadowSize, bw + 2 * rectShadowSize, bh + 2 * rectShadowSize, {
        ...
        var boxWidth = Math.max(textWidth, chunkTo) - Math.min(0, chunkFrom);
        // if (hasLeftArcs) {
      ...
      $.each(data.arcs, function (arcNo, arc) {
        ...
        for (var rowIndex = leftRow; rowIndex <= rightRow; rowIndex++) {
          ...
          var arrowName = (leftToRight ? symmetric && myArrowHead || 'none' : myArrowHead || 'triangle,5') + ',' + color;
          ...
          var myArrowHead = arcDesc && arcDesc.arrowHead || spanTypes.ARC_DEFAULT && spanTypes.ARC_DEFAULT.arrowHead;
          var arrowName = (leftToRight ? myArrowHead || 'triangle,5' : symmetric && myArrowHead || 'none') + ',' + color;
          var arrowType = arrows[arrowName];
          var arrowDecl = arrowType && 'url(#' + arrowType + ')';
      ...
      $.each(rows, function (rowId, row) {
        ...
        if (row.sentence) {
          ...
          var text = svg.text(link, sentNumMargin - Configuration.visual.margin.x, y - rowPadding, '' + row.sentence, { 'data-sent': row.sentence });
          ...
          if (sentComment) {
            ...
            var text = svg.text(sentNumGroup, sentNumMargin - Configuration.visual.margin.x, y - rowPadding, '' + row.sentence, { 'data-sent': row.sentence });
          }
        }
    ...
    if (!Visualizer.areFontsLoaded) {
      var webFontConfig = {
        custom: {
          families: ['Astloch', 'PT Sans Caption',
          //        'Ubuntu',
          'Liberation Sans'],
          /* For some cases, in particular for embedding, we need to
            allow for fonts being hosted elsewhere */
          urls: webFontURLs !== undefined ? webFontURLs : ['static/fonts/Astloch-Bold.ttf', 'static/fonts/PT_Sans-Caption-Web-Regular.ttf',
          //
