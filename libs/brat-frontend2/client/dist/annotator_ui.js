...
var AnnotatorUI = function ($, window, undefined) {
  var AnnotatorUI = function AnnotatorUI(dispatcher, svg) {
    var _initForm;
    ...
    var stripNumericSuffix = function stripNumericSuffix(s) {
    ...
    var makeSelRect = function makeSelRect(rx, ry, rw, rh, col) {
    ...
    var onKeyDown = function onKeyDown(evt) {
      ...
      if (Configuration.rapidModeOn && rapidAnnotationDialogVisible && "0".charCodeAt() <= code && code <= "9".charCodeAt()) {
    ...
    var onDblClick = function onDblClick(evt) {
      // must be logged in
    ...
    var startArcDrag = function startArcDrag(originId) {
      clearSelection();
    ...
    var getValidArcTypesForDrag = function getValidArcTypesForDrag(targetId, targetType) {
    ...
    var onMouseMove = function onMouseMove(evt) {
      if (arcDragOrigin) {
        ...
        var path = svg.createPath().move(arcDragOriginBox.center, arcDragOriginBox.y).curveC(arcDragOriginBox.center - dx, y, mx + dx, y, mx, my);
        ...
        if (chunkIndexFrom === undefined && chunkIndexTo === undefined && $(sel.anchorNode).attr('data-chunk-id') && $(sel.focusNode).attr('data-chunk-id')) {
          ...
          if (selRect == null) {
            ...
            if (startRec.x != lastStartRec.x && endRec.x != lastEndRec.x && (startRec.y != lastStartRec.y || endRec.y != lastEndRec.y)) {
              if (startRec.y < lastStartRec.y) {
                ...
                selRect[selRect.length - 1].setAttributeNS(null, "x", parseFloat(selRect[selRect.length - 1].getAttributeNS(null, "x")) + parseFloat(selRect[selRect.length - 1].getAttributeNS(null, "width")) - lastEndRec.width);
            ...
            if (endRec.y != lastEndRec.y || startRec.y != lastStartRec.y) {
              ...
              if (trunc) {
                ...
                if (flip) {
                  ...
                  if (startRec.y == endRec.y) {
                    ...
                    rw = parseFloat(activeSelRect.getAttributeNS(null, "x")) + parseFloat(activeSelRect.getAttributeNS(null, "width")) - startRec.x;
                  }
                ...
                if (flip) {
                  lastSel.setAttributeNS(null, "width", parseFloat(lastSel.getAttributeNS(null, "x")) + parseFloat(lastSel.getAttributeNS(null, "width")) - endBox.x);
                  lastSel.setAttributeNS(null, "x", endBox.x);
                } else {
                  lastSel.setAttributeNS(null, "width", startBox.x + startBox.width - parseFloat(lastSel.getAttributeNS(null, "x")));
                }
                var rx = 0;
                var ry = 0;
                var rw = 0;
              ...
              if (flip) {
                var rw = parseFloat(activeSelRect.getAttributeNS(null, "x")) + parseFloat(activeSelRect.getAttributeNS(null, "width")) - startRec.x;
                activeSelRect.setAttributeNS(null, "x", startRec.x);
                activeSelRect.setAttributeNS(null, "y", startRec.y);
                activeSelRect.setAttributeNS(null, "width", rw);
              } else {
    ...
    var adjustToCursor = function adjustToCursor(evt, element, centerX, centerY) {
    ...
    var updateCheckbox = function updateCheckbox($input) {
      var $widget = $input.button('widget');
      var $textspan = $widget.find('.ui-button-text');
      $textspan.html(($input[0].checked ? '&#x2611; ' : '&#x2610; ') + $widget.attr('data-bare'));
    };

    var fillSpanTypesAndDisplayForm = function fillSpanTypesAndDisplayForm(evt, spanText, span) {
      keymap = spanKeymap;
      ...
      if (!span) {
        ...
        var attrCategoryAndTypes = [['entity', entityAttributeTypes], ['event', eventAttributeTypes]];
      ...
      var showValidNormalizationsFor = function showValidNormalizationsFor(type) {
      ...
      showValidNormalizations = function showValidNormalizations() {
      ...
      var showAttributesFor = function showAttributesFor(attrTypes, category, type) {
      ...
      showValidAttributes = function showValidAttributes() {
    ...
    var submitReselect = function submitReselect() {
      $(reselectedSpan.rect).removeClass('reselect');
      reselectedSpan = null;
      spanForm.submit();
    };

    var rapidFillSpanTypesAndDisplayForm = function rapidFillSpanTypesAndDisplayForm(start, end, text, types) {
      ...
      $.each(types, function (typeNo, typeAndProb) {
        ...
        if (typeHotkey) {
          var name = $label.html();
          var replace = true;
          name = name.replace(new RegExp("(&[^;]*?)?(" + typeHotkey + ")", 'gi'), function (all, entity, letter) {
            if (replace && !entity) {
              replace = false;
              var hotkey = typeHotkey.toLowerCase() == letter ? typeHotkey.toLowerCase() : typeHotkey.toUpperCase();
      ...
      adjustToCursor(lastRapidAnnotationEvent, rapidSpanForm.parent(), false, false);
    };

    var clearSpanNotes = function clearSpanNotes(evt) {
      $('#span_notes').val('');
    };
    $('#clear_notes_button').button();
    $('#clear_notes_button').click(clearSpanNotes);

    var clearSpanNorm = function clearSpanNorm(evt) {
      clearNormalizationUI();
    };
    ...
    var setSpanNormText = function setSpanNormText(response) {
    ...
    // update link
    var spanNormDbUpdate = function spanNormDbUpdate(evt) {
      clearNormalizationUI();
      updateNormalizationDbLink();
    };
    ...
    // reference
    var spanNormIdUpdate = function spanNormIdUpdate(evt) {
      var key = $(this).val();
    ...
    initForm(normSearchDialog, (_initForm = {
      width: 800
    }, _defineProperty(_initForm, 'width', 600), _defineProperty(_initForm, 'resizable', true), _defineProperty(_initForm, 'alsoResize', '#norm_search_result_select'), _defineProperty(_initForm, 'open', function open(evt) {
      keymap = {};
    }), _defineProperty(_initForm, 'close', function close(evt) {
    ...
    */
    ...
    var normSubmit = function normSubmit(selectedId, selectedTxt) {
    ...
    var normSearchSubmit = function normSearchSubmit(evt) {
    ...
    var chooseNormIdAndSubmit = function chooseNormIdAndSubmit(evt) {
      chooseNormId(evt);
      normSearchSubmit(evt);
    };
    var setSpanNormSearchResults = function setSpanNormSearchResults(response) {
      if (response.exception) {
        // TODO: better response to failure
        dispatcher.post('messages', [[['Lookup error', 'warning', -1]]]);
        return false;
      ...
      $.each(response.items, function (itemNo, item) {
        ...
        // this workaround OK?
        html.push('<tr' + ' data-id="' + Util.escapeHTMLandQuotes(item[0]) + '"' + ' data-txt="' + Util.escapeHTMLandQuotes(item[1]) + '"' + '>');
        for (var i = 0; i < len; i++) {
    ...
    var performNormSearch = function performNormSearch() {
    ...
    var showNormSearchDialog = function showNormSearchDialog() {
      ...
      if (!$('#span_norm_id').val().match(/^\s*$/) && !$('#span_norm_txt').val().match(/^\s*$/)) {
    ...
    var arcFormSubmitRadio = function arcFormSubmitRadio(evt) {
      // TODO: check for confirm_mode?
      arcFormSubmit(evt, $(evt.target));
    };

    var arcFormSubmit = function arcFormSubmit(evt, typeRadio) {
    ...
    var fillArcTypesAndDisplayForm = function fillArcTypesAndDisplayForm(evt, originType, targetType, arcType, arcId) {
      var noArcs = true;
      keymap = {};
      ...
      var isEquiv = relationTypesHash && relationTypesHash[noNumArcType] && relationTypesHash[noNumArcType].properties && relationTypesHash[noNumArcType].properties.symmetric && relationTypesHash[noNumArcType].properties.transitive;
      ...
      if (spanTypes[originType]) {
        ...
        $.each(arcTypes || [], function (arcTypeNo, arcDesc) {
          if (arcDesc.targets && arcDesc.targets.indexOf(targetType) != -1) {
            ...
            var isThisEquiv = relationTypesHash && relationTypesHash[arcTypeName] && relationTypesHash[arcTypeName].properties && relationTypesHash[arcTypeName].properties.symmetric && relationTypesHash[arcTypeName].properties.transitive;

            // do not allow equiv<->non-equiv change options
            if (arcType && isEquiv != isThisEquiv) return;

            var displayName = arcDesc.labels && arcDesc.labels[0] || arcTypeName;
            ...
            if (arcDesc.hotkey) {
              ...
              var name = $label.html();
              var replace = true;
              name = name.replace(new RegExp("(&[^;]*?)?(" + arcDesc.hotkey + ")", 'gi'), function (all, entity, letter) {
                if (replace && !entity) {
                  replace = false;
                  var hotkey = arcDesc.hotkey.toLowerCase() == letter ? arcDesc.hotkey.toLowerCase() : arcDesc.hotkey.toUpperCase();
                  return '<span class="accesskey">' + Util.escapeHTML(hotkey) + '</span>';
                }
                return all;
              });
      ...
      if (noArcs) {
        if (arcId) {
          ...
          dispatcher.post('messages', [[["No choices for " + Util.spanDisplayForm(spanTypes, originType) + " -> " + Util.spanDisplayForm(spanTypes, targetType), 'warning']]]);
          return;
        }
      }
    ...
    var reverseArc = function reverseArc(evt) {
    ...
    var deleteArc = function deleteArc(evt) {
    ...
    var reselectArc = function reselectArc(evt) {
      dispatcher.post('hideForm');
    ...
    var stopArcDrag = function stopArcDrag(target) {
      if (arcDragOrigin) {
        if (!target) {
          target = $('.badTarget');
        }
    ...
    var onMouseUp = function onMouseUp(evt) {
      ...
      if (arcDragOrigin) {
        ...
        if (chunkIndexFrom !== undefined && chunkIndexTo !== undefined) {
          ...
          // trim
          while (selectedFrom < selectedTo && " \n\t".indexOf(data.text.substr(selectedFrom, 1)) !== -1) {
            selectedFrom++;
          }while (selectedFrom < selectedTo && " \n\t".indexOf(data.text.substr(selectedTo - 1, 1)) !== -1) {
            selectedTo--;
    ...
    var receivedSuggestedSpanTypes = function receivedSuggestedSpanTypes(sugg) {
      if (sugg.exception) {
    ...
    var toggleCollapsible = function toggleCollapsible($el, state) {
    ...
    var collapseHandler = function collapseHandler(evt) {
      toggleCollapsible($(evt.target));
    };

    var spanFormSubmitRadio = function spanFormSubmitRadio(evt) {
      if (Configuration.confirmModeOn) {
        showValidAttributes();
        showValidNormalizations();
        $('#span_form-ok').focus();
    ...
    var rapidSpanFormSubmitRadio = function rapidSpanFormSubmitRadio(evt) {
      rapidSpanFormSubmit(evt, $(evt.target));
    };

    var rememberData = function rememberData(_data) {
      if (_data && !_data.exception) {
        data = _data;
      }
    };

    var addSpanTypesToDivInner = function addSpanTypesToDivInner($parent, types, category) {
      if (!types) return;
      ...
      $.each(types, function (typeNo, type) {
        if (type === null) {
          ...
          if (type.hotkey) {
            ...
            var name = $label.html();
            var replace = true;
            name = name.replace(new RegExp("(&[^;]*?)?(" + type.hotkey + ")", 'gi'), function (all, entity, letter) {
              if (replace && !entity) {
                replace = false;
                var hotkey = type.hotkey.toLowerCase() == letter ? type.hotkey.toLowerCase() : type.hotkey.toUpperCase();
                return '<span class="accesskey">' + Util.escapeHTML(hotkey) + '</span>';
              }
              return all;
            });
    ...
    var addSpanTypesToDiv = function addSpanTypesToDiv($top, types, heading) {
    ...
    var addAttributeTypesToDiv = function addAttributeTypesToDiv($top, types, category) {
      $.each(types, function (attrNo, attr) {
        ...
        if (attr.unused) {
          ...
          var $label = $('<label class="attribute_type_label" for="' + attrId + '" data-bare="' + escapedName + '">&#x2610; ' + escapedName + '</label>');
    ...
    var setSpanTypeSelectability = function setSpanTypeSelectability(category) {
    ...
    var onMultiAttrChange = function onMultiAttrChange(evt) {
      if ($(this).val() == '') {
        $('#span_form input:not([unused])').removeAttr('disabled');
      } else {
    ...
    var onBooleanAttrChange = function onBooleanAttrChange(evt) {
      var attrCategory = evt.target.getAttribute('category');
      setSpanTypeSelectability(attrCategory);
      updateCheckbox($(evt.target));
    };

    var rememberSpanSettings = function rememberSpanSettings(response) {
      spanKeymap = {};
    ...
    var tagCurrentDocument = function tagCurrentDocument(taggerId) {
      var tagOptions = {
        action: 'tag',
        collection: coll,
        'document': doc,
    ...
    var setupTaggerUI = function setupTaggerUI(response) {
    ...
    var rememberNormDbsForType = function rememberNormDbsForType(types) {
      if (!types) return;
    ...
    var setupNormalizationUI = function setupNormalizationUI(response) {
      ...
      $.each(norm_resources, function (normNo, norm) {
        var normName = norm[0],
            ...
            normUrlBase = norm[2];
        var serverDb = norm[3];
        html.push('<option value="' + Util.escapeHTML(normName) + '">' + Util.escapeHTML(normName) + '</option>');
    ...
    var hideNormalizationRefLink = function hideNormalizationRefLink() {
    ...
    var updateNormalizationRefLink = function updateNormalizationRefLink() {
      var $normId = $('#span_norm_id');
    ...
    var updateNormalizationDbLink = function updateNormalizationDbLink() {
    ...
    var clearNormalizationUI = function clearNormalizationUI() {
      var $normId = $('#span_norm_id');
      var $normText = $('#span_norm_txt');
      $normId.val('');
      oldSpanNormIdValue = '';
    ...
    var spanNormalizations = function spanNormalizations() {
    ...
    // the span dialog
    var spanAttributes = function spanAttributes(typeRadio) {
    ...
    var spanAndAttributeTypesLoaded = function spanAndAttributeTypesLoaded(_spanTypes, _entityAttributeTypes, _eventAttributeTypes, _relationTypesHash) {
      ...
      // for easier access
      allAttributeTypes = $.extend({}, entityAttributeTypes, eventAttributeTypes);
    };

    var gotCurrent = function gotCurrent(_coll, _doc, _args) {
      coll = _coll;
      doc = _doc;
      args = _args;
    };
    ...
    var undoStack = [];
    var edited = function edited(response) {
    ...
    var reselectSpan = function reselectSpan() {
      dispatcher.post('hideForm');
      svgElement.addClass('reselect');
    ...
    var splitSpan = function splitSpan() {
      dispatcher.post('hideForm');
      var $roles = $('#split_roles').empty();
      var numRoles = repeatingArcTypes.length;
      var roles = $.each(repeatingArcTypes, function () {
        var $role = $('<input id="split_on_' + Util.escapeQuotes(this) + '" type="checkbox" name="' + Util.escapeQuotes(this) + '" value="' + Util.escapeQuotes(this) + '"/>');
        if (numRoles == 1) {
          // a single role will be selected automatically
          $role.click();
        }
        var $label = $('<label for="split_on_' + Util.escapeQuotes(this) + '">' + Util.escapeQuotes(this) + '</label>');
    ...
    var addFragment = function addFragment() {
      dispatcher.post('hideForm');
      svgElement.addClass('reselect');
      $(editedSpan.rect).addClass('reselect');
      reselectedSpan = editedSpan;
    ...
    var reselectFragment = function reselectFragment() {
      addFragment();
      selectedFragment = editedFragment;
    };

    var deleteFragment = function deleteFragment() {
    ...
    var spanFormSubmit = function spanFormSubmit(evt, typeRadio) {
      typeRadio = typeRadio || $('#span_form input:radio:checked');
      var type = typeRadio.val();
      $('#span_form-ok').blur();
      dispatcher.post('hideForm');
    ...
    var rapidSpanFormSubmit = function rapidSpanFormSubmit(evt, typeRadio) {
      ...
      if (type == "") {
        ...
        fillSpanTypesAndDisplayForm(lastRapidAnnotationEvent, $('#rapid_span_selected').text());
    ...
    var importFormSubmit = function importFormSubmit(evt) {
    ...
    var userReceived = function userReceived(_user) {
      that.user = _user;
    };

    var setAnnotationSpeed = function setAnnotationSpeed(speed) {
      if (speed == 1) {
    ...
    var onNewSourceData = function onNewSourceData(_sourceData) {
    ...
    var collectionLoaded = function collectionLoaded(collData) {
      coll = collData;
    };

    var requestRenderData = function requestRenderData(docData) {
      doc = docData;
    };
