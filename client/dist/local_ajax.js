...
var LocalAjax = function ($, window, undefined) {
    var LocalAjax = function LocalAjax(dispatcher, maxFragmentLength) {
        var that = this;

        var findType = function findType(entityTypes, type) {
        ...
        var createAnnotation = function createAnnotation(data) {
            ...
            if (!e_type) {
                //Trigger
                e_type = data.collection.event_types.find(function (x) {
                ...
                var e_id = "N" + (that.document.entities.length + 1),
                    //TODO: must absolutely be unique
            ...
            for (var key in attrs) {
                if (attrs.hasOwnProperty(key) && attrs[key]) {
                    data.document.attributes.push(["A" + (that.document.attributes.length + 1), //TODO: must absolutely be unique,
                    key, e_id, attrs[key]]);
                }
            }
        ...
        var splitTooLongFragment = function splitTooLongFragment(offsets, data, e_id) {
            var new_offsets = [];

            if (maxFragmentLength > 0 && offsets.find(function (x) {
                return x[1] - x[0] > maxFragmentLength;
            })) {
                offsets.forEach(function (fragment) {
                    ...
                    if (to - from > maxFragmentLength) {
                        ...
                        data.document.attributes.push(["A" + (that.document.attributes.length + 1), //TODO: must absolutely be unique,
                        LONG_ANNOTATION_CONST, e_id, [from, to]]);
                    } else {
        ...
        var editAnnotation = function editAnnotation(data) {
            var e_type = {},
                //Entity or Trigger
            ...
            if (data.id.substring(0, 1) == "E") {
                //Event annotation
                //data.normalisations ??
                var annotation = data.document.events.find(function (x) {
                    return x[0] === data.id;
                });
                var trigger_id = annotation[1];
                var trigger = data.document.triggers.find(function (x) {
                    return x[0] === trigger_id;
                });
                trigger[1] = data.type;
                trigger[2] = offsets;
                e_type = data.collection.event_types.find(function (x) {
                ...
                //Entity annotation
                var entity = data.document.entities.find(function (x) {
                    return x[0] === data.id;
                });
                entity[1] = data.type;
            ...
            if (e_type) {
                ...
                var existing_attrs = data.document.attributes.filter(function (x) {
                    return x[2] === data.id;
                });
                existing_attrs.forEach(function (attr) {
                    var index = data.document.attributes.indexOf(function (x) {
                        return x[0] === attr[0];
                ...
                for (var key in attrs) {
                    if (attrs.hasOwnProperty(key) && attrs[key]) {
                        existing_attrs.find(function (x) {
                            return x[1] === key;
                        });

                        data.document.attributes.push(["A" + (that.document.attributes.length + 1), //TODO: must absolutely be unique,
                        key, data.id, attrs[key]]);
                    }
                }

                //Add/Edit comment content
                if (data.comment.length) {
                    var comment = data.document.comments.find(function (x) {
                        return x[0] === data.id;
                    });
                    if (comment) {
                        //Edit
        ...
        var createRelation = function createRelation(data) {
            var e_type = data.collection.relation_types.find(function (x) {
                return x.type === data.type;
            }); //Entity or Event
            ...
            if (!e_type) {
                ...
                e_type = data.document.events.find(function (x) {
                    return x[0] === data.origin;
                });
                if (e_type) {
                ...
                //Entity relation
                var obj = ["R" + (that.document.relations.length + 1), //TODO: must absolutely me unique
                data.type, [[e_type.args[0].role, data.origin], [e_type.args[1].role, data.target]]];
        ...
        var editRelation = function editRelation(data) {
            var e_type = data.collection.relation_types.find(function (x) {
                return x.type === data.type;
            }); //Entity or Event
            ...
            if (!e_type) {
                ...
                //Entity relation
                var relation = data.document.relations.find(function (x) {
                    return x[1] === data.old_type && x[2][0][1] === data.origin && x[2][1][1] === data.old_target;
                });
                relation[1] = data.type;
                relation[2] = [[e_type.args[0].role, data.origin], [e_type.args[1].role, data.target]];
            }
        ...
        var localExecution = function localExecution(data, callback, merge) {
