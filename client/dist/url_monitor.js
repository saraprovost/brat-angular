...
var URLMonitor = function ($, window, undefined) {
  var URLMonitor = function URLMonitor(dispatcher) {
    var that = this;
    ...
    var setArguments = function setArguments(args, force) {
      ...
      if (changed) {
        ...
        dispatcher.post('hideForm');
        dispatcher.post('argsChanged', [args, oldArgs]);
        dispatcher.post('current', [that.url_hash.collection, that.url_hash.document, that.url_hash.arguments, reloadData]);
        reloadData = true;
      }
    };

    var setDocument = function setDocument(doc, args) {
    ...
    var setCollection = function setCollection(coll, doc, args) {
    ...
    var updateState = function updateState() {
      ...
      if (!changed) {
        ...
        setCollection(new_url_hash.collection, new_url_hash.document, $.extend(new_url_hash.arguments, newIntArgs));
    ...
    var preventReloadByURL = function preventReloadByURL() {
      reloadData = false;
    };
    var allowReloadByURL = function allowReloadByURL() {
      reloadData = true;
    };
    ...
    dispatcher.on('forceUpdate', forceUpdate).on('setArguments', setArguments).on('setDocument', setDocument).on('setCollection', setCollection).on('preventReloadByURL', preventReloadByURL).on('allowReloadByURL', allowReloadByURL).on('init', init);
  };
...
var URLHash = function ($, window, undefined) {
  var URLHash = function URLHash(collection, _document, _arguments) {
    var that = this;
  ...
  URLHash.prototype = {
    ...
    setArguments: function setArguments(_arguments) {
    ...
    setDocument: function setDocument(_document) {
