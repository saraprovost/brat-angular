...
var Util = function (window, undefined) {
  ...
  var cmpArrayOnFirstElement = function cmpArrayOnFirstElement(a, b) {
    a = a[0];
    b = b[0];
    return a < b ? -1 : a > b ? 1 : 0;
  };

  var unitAgo = function unitAgo(n, unit) {
  ...
  var realBBox = function realBBox(span) {
  ...
  var escapeHTMLandQuotes = function escapeHTMLandQuotes(str) {
    return str.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\"/g, '&quot;');
  };

  var escapeHTMLwithNewlines = function escapeHTMLwithNewlines(str) {
  ...
  var getSpanLabels = function getSpanLabels(spanTypes, spanType) {
    var type = spanTypes[spanType];
    return type && type.labels || [];
  };

  var spanDisplayForm = function spanDisplayForm(spanTypes, spanType) {
    var labels = getSpanLabels(spanTypes, spanType);
    return labels[0] || spanType;
  };

  var getArcLabels = function getArcLabels(spanTypes, spanType, arcType, relationTypesHash) {
    var type = spanTypes[spanType];
  ...
  var arcDisplayForm = function arcDisplayForm(spanTypes, spanType, arcType, relationTypesHash) {
  ...
  var objectToUrlStr = function objectToUrlStr(o) {
    a = [];
  ...
  var strToRgb = function strToRgb(color) {
    var result;
    ...
    if (result = rgbNumRE.exec(color)) return [parseInt(result[1]), parseInt(result[2]), parseInt(result[3])];

    // Look for rgb(num%,num%,num%)
    if (result = rgbPercRE.exec(color)) return [parseFloat(result[1]) * 2.55, parseFloat(result[2]) * 2.55, parseFloat(result[3]) * 2.55];

    // Look for #a0b1c2
    if (result = rgbHash6RE.exec(color)) return [parseInt(result[1], 16), parseInt(result[2], 16), parseInt(result[3], 16)];

    // Look for #fff
    if (result = rgbHash3RE.exec(color)) return [parseInt(result[1] + result[1], 16), parseInt(result[2] + result[2], 16), parseInt(result[3] + result[3], 16)];

    // Otherwise, we're most likely dealing with a named color
    return colors[$.trim(color).toLowerCase()];
  };

  var rgbToStr = function rgbToStr(rgb) {
  ...
  // the given color.
  var adjustColorLightness = function adjustColorLightness(colorstr, adjust) {
  ...
  var isEqual = function isEqual(a, b) {
    ...
    var atype = typeof a === 'undefined' ? 'undefined' : _typeof(a),
        btype = typeof b === 'undefined' ? 'undefined' : _typeof(b);
  ...
  var deparam = function deparam(str) {
    var args = str.split('&');
    var len = args.length;
    if (!len) return null;
    var result = {};
  ...
  var paramArray = function paramArray(val) {
    val = val || [];
    var len = val.length;
    var arr = [];
    for (var i = 0; i < len; i++) {
  ...
  var profileStarts = {};
  var profileOn = false;
  var profileEnable = function profileEnable(on) {
    if (on === undefined) on = true;
    profileOn = on;
  }; // profileEnable
  var profileClear = function profileClear() {
    if (!profileOn) return;
    profiles = {};
    profileStarts = {};
  }; // profileClear
  var profileStart = function profileStart(label) {
    if (!profileOn) return;
    profileStarts[label] = new Date();
  }; // profileStart
  var profileEnd = function profileEnd(label) {
    if (!profileOn) return;
    var profileElapsed = new Date() - profileStarts[label];
    if (!profiles[label]) profiles[label] = 0;
    profiles[label] += profileElapsed;
  }; // profileEnd
  var profileReport = function profileReport() {
    if (!profileOn) return;
    if (window.console) {
  ...
  var embed = function embed(container, collData, docData, webFontURLs) {
  ...
  var embedByURL = function embedByURL(container, collDataURL, docDataURL, callback) {
